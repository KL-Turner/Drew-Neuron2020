...
f=getfgrid_Neuron2020(Fs,nfft,fpass); Nf=length(f);
params.tapers=dpsschk_Neuron2020(tapers,Nwin,Fs); % check tapers
...
for n=1:nw;
   ...
   datawin=data(indx,:);
   if nargout==4
     [s,f,serr]=mtspectrumc_Neuron2020(datawin,params);
     Serr(1,n,:,:)=squeeze(serr(1,:,:));
     Serr(2,n,:,:)=squeeze(serr(2,:,:));
   else
     [s,f]=mtspectrumc_Neuron2020(datawin,params);
   end
   S(n,:,:)=s;
end;
S=squeeze(S); 
