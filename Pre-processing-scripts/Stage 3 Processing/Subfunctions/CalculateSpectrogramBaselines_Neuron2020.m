function [RestingBaselines] = CalculateSpectrogramBaselines_Neuron2020(animal, trialDuration_Sec, specDataFiles, RestingBaselines)
...
dsFs = 30;
restFileList = unique(RestingBaselines.baselineFileInfo.fileIDs);
...
for a = 1:length(restFileList)
    ...
    for b = 1:size(specDataFiles,1)
        [~,~,specDataFile,~,~] = GetFileInfo2_Neuron2020(specDataFiles(b,:));
...
for c = 1:length(restFileList)
    ...
    strDay = ConvertDate_Neuron2020(fileID(1:6));
    S1_data = restS1{c,1};
    S5_data = restS5{c,1};
...
uniqueDays = GetUniqueDays_Neuron2020(RestingBaselines.baselineFileInfo.fileIDs);
for e = 1:length(uniqueDays)
    f = 1;
    for field = 1:length(fields)
        if strcmp(fields{field}(7:12),uniqueDays{e})
            stringDay = ConvertDate_Neuron2020(uniqueDays{e});
