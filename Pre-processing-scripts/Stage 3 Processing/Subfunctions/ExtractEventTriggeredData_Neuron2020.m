function [EventData] = ExtractEventTriggeredData_Neuron2020(mergedDataFiles, dataTypes)
...
for dT = 1:length(dataTypes)
    ...
    for f = 1:size(mergedDataFiles,1)    
        ...
        [animalID,fileDate,fileID,vesselID, ~] = GetFileInfo2_Neuron2020(mergedDataFiles(f,:));
        ...
        if strcmp(dataType, 'vesselDiameter')
            ...
            for bF = 1:length(behaviorFields)
                ...
                [chunkData,evFilter] = ExtractBehavioralData_Neuron2020(data,epoch,dataType,Fs,behaviorFields{bF});
                % Add epoch details to temp struct
                [temp] = AddEpochInfo_Neuron2020(data, behaviorFields{bF},temp,fileID,fileDate,vesselID,evFilter,f);
                temp.(behaviorFields{bF}).data{f} = chunkData;
            end
    end
    % Convert the temporary stuct into a final structure
    [EventData] = ProcessTempStruct_Neuron2020(EventData,dataType,temp,epoch);
end
save([animalID '_EventData.mat'],'EventData');

function [chunkData, evFilter] = ExtractBehavioralData_Neuron2020(data,epoch,dataType,Fs,behavior)
% Setup variables
...
stopFilter = round(allEpochEnds) < trialDuration; % Apply "round" to give an extra half second buffer to prevent indexing errors
...
% potential matrix dimension erros caused by differences in rounding when converting from seconds to samples.
...
function [temp] = AddEpochInfo_Neuron2020(data,behavior,temp,fileID,fileDate,vesselID,evFilter,f)
...
function [EventData] = ProcessTempStruct_Neuron2020(EventData,dataType,temp,epoch)
% Get dataType names
